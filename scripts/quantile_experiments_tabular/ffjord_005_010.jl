...
modelname = "ffjord_005_010"

function sample_params()
	parameter_rng = (
        activation = [tanh, σ, elu, softplus, identity],
        nhidden = 1:4,
        size_hidden = 5:5:20,
        nblocks = 1:2:9,
        batchsize = 2 .^ (5:7),
...
function fit(data, parameters)
	model = GenerativeAD.Models.FFJORD(;idim=size(data[1][1],1), parameters...)
...
data = GenerativeAD.load_data("abalone", seed=1, contamination=0.0)
pr = sample_params()
